...
def run_gesture_recognition(target_gestures):
    ...
    for gesture, direction in predict_gesture_and_direction(cap, payload.get_model_path(),
        ...
        if intended_combined_gesture is not None and 10 <= intended_combined_gesture <= 50:
...
print("Starting second phase with target gestures: {10, 11, 12, 13}")
second_phase_results = run_gesture_recognition({10, 11, 12, 13})
...
print("Starting first phase with target gestures: {42, 43, 44, 45}")
first_phase_results = run_gesture_recognition({42, 43, 44, 45})
...
print(f"Final Total Dynamic Predictions: {first_phase_results[2]}")
...
print(f"Final Total Dynamic Predictions: {second_phase_results[2]}")
