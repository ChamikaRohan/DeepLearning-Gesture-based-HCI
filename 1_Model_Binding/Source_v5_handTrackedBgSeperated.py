        ...
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            if x_min < x_max and y_min < y_max:
                ...
                if cropped_hand.size != 0:
                    ...
                    hand_points = [(int(landmark.x * cropped_hand.shape[1]), int(landmark.y * cropped_hand.shape[0])) for landmark in hand_landmarks.landmark]
                    cv2.fillConvexPoly(mask, np.array(hand_points[:21], dtype=np.int32), (255, 255, 255))
                    ...
                    gray_hand = cv2.cvtColor(masked_frame, cv2.COLOR_BGR2GRAY)

                    # Apply adaptive thresholding
                    binary_hand = cv2.adaptiveThreshold(gray_hand, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 11, 2)

                    # Invert the binary image
                    binary_hand = cv2.bitwise_not(binary_hand)

                    # Apply morphological operations to clean up the image
                    kernel = np.ones((5, 5), np.uint8)
                    binary_hand = cv2.morphologyEx(binary_hand, cv2.MORPH_OPEN, kernel)
                    binary_hand = cv2.morphologyEx(binary_hand, cv2.MORPH_CLOSE, kernel)
